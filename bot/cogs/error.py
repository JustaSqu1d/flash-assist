...
class Error(discord.Cog):
    ...
    async def on_error(self, event: str = None, args=None, kwargs=None) -> None:
    ...
    async def on_application_command_error(
        ...
        embed = discord.Embed(
            title="A superduper rare error occured.",
            description="Meanwhile, we have begun to debug this bug. Everything should work iceman perfect. You can also check our status page if there is an outage.",
        )
        ...
        error = discord.Embed(
            title=f"Error at {ctx.command.qualified_name}!", color=discord.Color.red()
        )
        error.add_field(name="User", value=f"{ctx.author} ({ctx.author.id})")
        try:
            error.add_field(
                name="Guild/Channel", value=f"{ctx.guild.name}/{ctx.channel.mention}"
            )
        except:
            pass
        ...
        error.add_field(
            ...
            value=f"{ctx.interaction.is_command()}/{ctx.interaction.is_component()}",
        )
        ...
        error.add_field(
            name="Exception", value=f"{exception}: {exception.__traceback__}"
        )
