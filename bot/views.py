...
class Invite(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            ...
            url="https://discord.com/api/oauth2/authorize?client_id=836581672811495465&permissions=8590322688&redirect_uri=https%3A%2F%2Fdiscord.com%2Finvite%2FfJt6yFeD5v&response_type=code&scope=bot%20applications.commands%20identify",
            row=1,
        )
...
class Invite2(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            ...
            style=discord.ButtonStyle.link,
            url="https://discord.com/api/oauth2/authorize?client_id=836581672811495465&permissions=8590322688&redirect_uri=https%3A%2F%2Fflash-assist.glitch.me%2F&response_type=code&scope=bot%20applications.commands%20identify",
            row=1,
        )
...
class Invite3(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            label="Website",
            style=discord.ButtonStyle.link,
            url="https://flash-assist.glitch.me/",
            row=1,
        )
...
class Invite4(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            ...
            style=discord.ButtonStyle.link,
            url="https://discord.com/invite/fJt6yFeD5v",
            row=2,
        )
...
class Status(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            label="Bot Status",
            style=discord.ButtonStyle.link,
            url="https://flashassist.statuspage.io/",
            row=2,
        )
...
class Vote(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            label="Vote for me!",
            style=discord.ButtonStyle.link,
            url="https://top.gg/bot/836581672811495465/vote",
            row=2,
        )
...
class TogglesCl(discord.ui.View):
    ...
    async def callback(
        ...
        self.ctx.bot.minecordclassic.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"mine": not (self.db["minecordclassic"]["mine"])}},
        )
    ...
    async def callback2(
        ...
        self.ctx.bot.minecordclassic.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"fight": not (self.db["minecordclassic"]["fight"])}},
        )
        await interaction.response.edit_message(view=self)
    ...
    async def callback3(
        ...
        self.ctx.bot.minecordclassic.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"chop": not (self.db["minecordclassic"]["chop"])}},
        )
        await interaction.response.edit_message(view=self)
    ...
    async def callback4(
        ...
        self.ctx.bot.minecordclassic.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"ed": not (self.db["minecordclassic"]["ed"])}},
        )
        await interaction.response.edit_message(view=self)

    @discord.ui.select(
        placeholder="Currently editing: Minecord Classic",
        min_values=1,
        max_values=1,
        options=[
            discord.SelectOption(
                label="Virtual Fisher", description="Edit Virtual Fisher Settings"
            ),
            discord.SelectOption(
                label="Minecord", description="Edit Minecord Classic Settings"
            ),
        ],
    )
...
class Toggles(discord.ui.View):
    ...
    async def callback(
        ...
        self.ctx.bot.minecord.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"mine": not (self.db["minecord"]["mine"])}},
        )
        await interaction.response.edit_message(view=self)
    ...
    async def callback2(
        ...
        self.ctx.bot.minecord.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"fight": not (self.db["minecord"]["fight"])}},
        )
        await interaction.response.edit_message(view=self)
    ...
    async def callback3(
        ...
        self.ctx.bot.minecord.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"chop": not (self.db["minecord"]["chop"])}},
        )
        await interaction.response.edit_message(view=self)
    ...
    async def callback4(
        ...
        self.ctx.bot.minecord.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"ed": not (self.db["minecord"]["ed"])}},
        )
        await interaction.response.edit_message(view=self)

    @discord.ui.select(
        placeholder="Currently editing: Minecord",
        min_values=1,
        max_values=1,
        options=[
            discord.SelectOption(
            ...
            discord.SelectOption(
                label="Minecord Classic", description="Edit Minecord Classic Settings"
            ),
        ],
    )
    async def select_callback(self, select, interaction):
...
class TogglesVf(discord.ui.View):
    ...
    async def callback(
        ...
        self.ctx.bot.virtualfisher.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"treasure": not (self.db["virtualfisher"]["treasure"])}},
        )
        await interaction.response.edit_message(view=self)
    ...
    async def callback2(
        ...
        self.ctx.bot.virtualfisher.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"fish": not (self.db["virtualfisher"]["fish"])}},
        )
        await interaction.response.edit_message(view=self)
    ...
    async def callback3(
        ...
        self.ctx.bot.virtualfisher.update_one(
            {"_id": self.ctx.author.id},
            {"$set": {"worker": not (self.db["virtualfisher"]["worker"])}},
        )
        await interaction.response.edit_message(view=self)

    @discord.ui.select(
        placeholder="Currently editing: Virtual Fisher",
        min_values=1,
        max_values=1,
        options=[
            discord.SelectOption(
                label="Minecord", description="Edit Minecord Settings"
            ),
            discord.SelectOption(
                label="Minecord Classic", description="Edit Minecord Classic Settings"
            ),
...
class Event(Modal):
    def __init__(self) -> None:
        ...
        self.add_item(
            InputText(
                label="How long do you want the event to last?",
